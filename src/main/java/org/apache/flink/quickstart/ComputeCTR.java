...
public class ComputeCTR extends BaseStreamingExample {
    public static void main(String [] args) throws Exception {
        ...
        tblEnv.connect(new Kafka().version("0.10")
                ...
                .withSchema(new Schema().schema(TableSchema.fromTypeInfo(AvroSchemaConverter.convertToTypeInfo(UserExpose.class)))
        ...
        tblEnv.connect(new Kafka().version("0.10")
                ...
                .withSchema(new Schema().schema(TableSchema.fromTypeInfo(AvroSchemaConverter.convertToTypeInfo(UserClick.class)))
                        .field("tt", Types.SQL_TIMESTAMP).proctime())
                .inAppendMode()
        ...
        Table result = tblEnv.sqlQuery("SELECT exposeCount.source, exposeCount.sTime, clickCount.sCount / exposeCount.sCount\n" +
                "FROM\n" +
                "(SELECT source, TUMBLE_END(tt, INTERVAL '1' MINUTE) as sTime, CAST(count(1) AS DOUBLE) as sCount FROM expose GROUP BY source, TUMBLE(tt, INTERVAL '1' MINUTE)) AS exposeCount \n" +
                "LEFT JOIN\n" +
                "(SELECT source, TUMBLE_END(tt, INTERVAL '1' MINUTE) as sTime, CAST(count(1) AS DOUBLE) as sCount FROM click  GROUP BY source, TUMBLE(tt, INTERVAL '1' MINUTE)) AS clickCount\n" +
                "ON\n" +
